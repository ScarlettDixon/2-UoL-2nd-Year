...
namespace Pathfinder
{
    ...
    {   
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                level.dijk.build(level, bot, player);
               // if (level.dijk.dijk_Activate == true) { bot.SetNextGridPosition( ,level); }
        ...
        private void DrawGrid()
        {
            ...
            for (int x = 0; x < sz; x++)
            {
                for (int y = 0; y < sz; y++)
                {
                    ...
                    {
                        if (level.dijk.inPath[x, y])
                        {
                            spriteBatch.Draw(tile1Texture, pos, Color.Green);
                        }
                        else if (level.dijk.closed[x, y] == true)
                        {
